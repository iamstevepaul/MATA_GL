...
def getNextTask(currentTime, iRobot, robotState, robotNodes, taskNodes, \
                ...
                incentiveModelType=6, isFullTaskState=True, isVisualize=False, matching_algorithm="hungarian",
                pos=None):  # , alg='MFMC'):
    # robotState
    ...
    for i in range(nRobot):
        if (robotState[i, 3] > 0) and (
                robotState[i, 4] > 0):  # Check whether i-th robot has enough range or payload to compete for tasks
            ...
            if not isFullTaskState:
                print("Limited Case")
                delayTime = timeDeadline[taskNodes] - (currentTime + timeMatrix[k, taskNodes])
            ...
            for taskNode in allowedTasks_i:  # taskNodes: # it must be integer
                k = int(robotState[i, 0])
                finishTime = robotState[i, 1] + timeMatrix[k, taskNode]
                finishRange = robotState[i, 4] - distanceMatrix[k, taskNode] - distanceMatrix[taskNode, 0]
                if (finishTime <= timeDeadline[taskNode]) and (
                        finishRange >= 0):  # and ( (TotalTime*0.5 + currentTime) >= timeDeadline[taskNode] ):
    ...
    if isVisualize and currentTime > 0 and nx.is_connected(B):
        ...
        print(sol)
        fileName = 'Results_Illustration/Illustration_DecMataResults_t' + str(currentTime) + "_r" + str(
            iRobot) + "_nx" + str(nxLoc)
...
def getParameters(letancyTime=0, Q=5, Range=100, Vavg=24/60, timeMax=6 * 60, timeStep=1, decTime=0):
    ## Alg parameters
